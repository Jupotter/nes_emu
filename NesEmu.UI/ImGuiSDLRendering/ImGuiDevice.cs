...
public sealed unsafe class ImGuiDevice : IDisposable
{
	...
	public void Render(ImDrawDataPtr data)
	{
		...
		for (int i = 0; i < lists.Size; i++) ExecuteCommandList(lists[i], clipOffset, clipSize);
	...
	public IntPtr CreateTexture(int sizeX, int sizeY, bool streaming, bool interpolation = true, bool bigEndian = false)
	{
		...
		IntPtr texture = SDL_CreateTexture(renderer, format, access, sizeX, sizeY);
		if (texture == IntPtr.Zero) throw new SDLException("Failed to create texture.");
	...
	void AssignBackendNames()
	{
		...
		io.NativePtr->BackendRendererName = (byte*)Marshal.StringToHGlobalAnsi($"{name} {info.max_texture_width}x{info.max_texture_height}");
	}
	...
	void CreateFontTexture()
	{
		...
		fontTexture = CreateTexture(width, height, false);
